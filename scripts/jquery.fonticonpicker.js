	...
	Plugin.prototype = {
		...
		renderIconContainer: function () {
			...
			for (var i = 0, item; item = iconsPaged[i++];) {
				...
				// Set the icon box
				$('<span/>', {
					html:      '<i data-fip-value="' + item + '" ' + (this.settings.useAttribute ? (this.settings.attributeName + '="' + ( this.settings.convertToHex ? '&#x' + parseInt(item, 10).toString(16) + ';' : item ) + '"') : 'class="fa ' + item + '"') + '></i>',
					'class':   'fip-box',
					title: flipBoxTitle
		...
		setSelectedIcon: function (theIcon) {
			if (theIcon === 'fip-icon-block' || theIcon === '' || theIcon === null) {
				theIcon = '';
			}
			...
			// Use class
				...
				var iconClass = (theIcon === '' ? 'fip-icon-block' : 'fa '+ theIcon);
				this.iconPicker.find('.selected-icon').html('<i class="' + iconClass + '"></i>');
			}
